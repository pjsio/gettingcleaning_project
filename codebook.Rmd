...
Each row represents a non-redundant entry, and is organized first by `subjnum`(representing Volunteer ID) (30) and `activity` (6) for each volunteer (30*6=180 rows).
  ...
  3) Decoding the stats columns:

For example, in `taccmeanx`, `t` denotes the 'time'. `acc` denotes acceleration (whereas `gyro` denotes gyroscope 3-axial movements), `mean` and `x` denote the mean for the x axis.
